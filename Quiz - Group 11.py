...
#Sign up method
    def c_reg(self):
        ...
        self.app = Registeration(self.newWindow)
#login method
    def c_login(self):
...
class Registeration:
    ...
    def c_submit(self):
        ...
        conn = mysql.connector.connect(host='localhost', database='quizdatabase', user='root', password='mysql')  #### Connect using ampps server
        cursor = conn.cursor()
        ...
        if c == 1:
            #check whether there is already taken similer username
            str = "select * from register where uname='%s'"
            arg = (uname)
            ...
            if row is not None:
                ...
                try:
                    s = "insert into register(Firstname,Lastname,email,uname,password,score,user) values('%s','%s','%s','%s','%s','%d','%d')"
                    #s = "insert into register(name,lname,email,uname,p,score,user) values('qqq','aaa','ww@gmail.com','wwww','12345678','%d','%d')" 
...
#when user click on the login button call the click method
    ...
    def clicked(self):
        ...
        s = "select * from register where uname='%s' and password='%s'"
        s1= "select user from register where uname='%s' and password='%s'"
...
####   Admin User ----  Upload the CSV file to database -------------
...
class AdminWindow:
    ...
    def AssignQuestions(self):
        wb=open("Questionsnew1.csv",'r')  ########  CSV file with questions and answers
        sheet=csv.reader(wb)
...
#### Create question table ######        
        ...
        messagebox.showinfo("Success", "Questions Assign Sucessfully")
...
class Account:
    ...
    def __init__(self, mainWin, u ):
        ...
        #insert photo into Account  window 
...
#next method use to go up to 5th question and displing warning when try to go to next  
    ...
    def Submit(self):
       ...
       # conn = mysql.connector.connect(host='localhost', database='quizdatabase', user='root', password='')
        ...
        cursor = conn.cursor()
        q = "update register set score='%d' where uname= '%s'"
        arg = (s, self.user)
        ...
        conn.close()
#####################################################
        #Display Questions and the correct answer in the table with the total marks at the end of the quiz
        ...
        class showAnswerstable:
            ...
            def __init__(self,root):
                ...
                # code for creating table to display the user all questions--
                # --user faced and the correct answer of each question
                for i in range(6):
